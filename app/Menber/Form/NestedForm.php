...
class NestedForm
{
    ...
    protected function prepareRecord($record)
    {
        ...
        foreach ($this->fields as $field) {
            ...
            if (($field instanceof \App\Menber\Form\Field\Hidden) || $value != $field->original()) {
                if (is_array($columns)) {
    ...
    public function getTemplateHtmlAndScript()
    {
        ...
        foreach ($this->fields() as $field) {
            ...
            //when field render, will push $script to Menber
            $html .= $field->render();

            /*
             * Get and remove the last script of Menber::$script stack.
             */
            if ($field->getScript()) {
                $scripts[] = array_pop(Menber::$script);
            }
        }
